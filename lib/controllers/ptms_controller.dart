...
class PTMSController extends GetxController {
              ...
              if (playerTeam == null) {
                ...
                _apiClient.registerPlayerTeam(playerTeamNew, session.token).then((result) {
                  if (result['status'].toString() == '201') {
                    ...
                    for (var element in ptMembers) {
                      final userId = playerProfiles
                          .where((profile) {
                            // Log.i(player.defaultIgn);
                            return profile.id == element.playerProfileId;
                          })
                          .toList()
                          .first
                          .userId;
                      ...
                      _apiClient.getUserProfileById(userId, 'Enable', session.token).then((value) {
                        if (value!['status'].toString() == '200') {
                          ...
                          _apiClient.sendSMSToAssignedPlayers({
                            "message": "You are assigned to ${playerTeam.name}",
                            "desAddr": ["tel:${userProfile.mobile}"]
                          }, session.token).then((value) {
                            if (value['status'].toString() == '200') {
                              Log.i('SMS Send');
                            }
                          }).catchError((error) {
